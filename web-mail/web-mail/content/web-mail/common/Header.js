...
headers.prototype =
{
    ...
    decodeEncodedWord : function (szValue)
    {
    	...
    	//szValue = =?UTF-8?B?MjAxMS0wNzEyLeWQiOe0hOabuC1Ub21vcnJvdy3kv67mlLnlkIjntIQtM+eJiC5k?= =?UTF-8?B?b2M=?=
        ...
        if (szValue.search(/^=\?.*?\?=$/)!=-1)
        {
        	var aPart = szValue.split(/\s/);
        	for (k=0; k<aPart.length; k++)
        	{
        		var szTemp = aPart[k];
        		var aszEncoding = szTemp.match(/^=\?(.*?)\?(.*?)\?(.*?)\?=$/);
	            ...
	            szDecode += this.convertToUTF8(szDecoded, aszEncoding[1]);
